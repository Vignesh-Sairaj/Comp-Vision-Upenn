...
function [u,v] = multiscale_aux(I1,I2,l,lmax)
    ...
    I1_ = gauss_blur(imresize(I1, 0.5));
    I2_ = gauss_blur(imresize(I2, 0.5));
%     I1_ = imresize(gauss_blur(I1), 0.5);
%     I2_ = imresize(gauss_blur(I2), 0.5);
...
%     cols_w = cols+uf;
%     
%     warp(sub2ind([sy, sx], rows, cols)) = interp2(I, cols_w, rows_w); %I(sub2ind([sy, sx], rows_w, cols_w));
%     warp(isnan(warp)) = 0;
    u = round(u);
    v = round(v);
    for i = 1:sy
        for j = 1:sx
            warp(i, j) = I(max(0, min(sy, i+v(i, j))), max(0, min(sx, j+u(i, j))));
        end
    end
    warp((x+u>sx)|(y+v>sy)|(x+u<1)|(y+v<1)) = 0;
end
